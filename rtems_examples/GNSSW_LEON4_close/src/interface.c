...
#endif
{
	...
	while(i<=0)
            i=sendto(sd,buffer,1,0,(struct sockaddr *)&serveraddr, sockaddrlen);
}

void SendResultCalculation(char *BufferToSend, int LenToSend)
{
    int i=0;
   // printf("send calc len %d\n", LenToSend);
    while(i<=0)
	i=sendto(sd,BufferToSend,LenToSend,0,(struct sockaddr *)&serveraddr, sockaddrlen);
   // printf("sent done\n");
}
...
{            
       ...
       // int y=500;
        ...
        y.tv_sec = 0;  
        y.tv_usec=500000;
        if(setsockopt(sd, SOL_SOCKET, SO_RCVTIMEO,&y,sizeof(y))!=0)
       ...
       //     printf("receiving 1\n");
            ...
            recv_len=RecvTcp();
#else
            recv_len = recvfrom(sd, buffer, MAX_BUFFER_SIZE, 0, (struct sockaddr *)&serveraddr, &sockaddrlen);

         //   printf("send buffer 1 recv %d\n", recv_len); 
#endif 
        }
        recv_len=-1;
