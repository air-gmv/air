...
{
        rtems_event_set out __attribute__((unused));
        int i;
        int fd;
        int lstatus;
        int ret;
        ...
        if (ioctl(fd, SPACEWIRE_IOCTRL_SET_COREFREQ, 0) == -1) {
                printf("ioctl failed: SPACEWIRE_IOCTRL_SET_COREFREQ\n");
                exit(0);
        }
       ...
       if (ioctl(fd, SPACEWIRE_IOCTRL_GET_LINK_STATUS, &(lstatus)) == -1) {
        ...
        printf("Spacewire link 1 is in %s state\n", link_status(lstatus));
       	...
       	if (ioctl(fd, SPACEWIRE_IOCTRL_LINKSTART, 1) == -1) {
                printf("ioctl failed: SPACEWIRE_IOCTRL_LINKSTART\n");
                exit(0);
        }
	...
	if (ioctl(fd, SPACEWIRE_IOCTRL_START, 0) == -1) {
            printf("ioctl failed: SPACEWIRE_IOCTRL_START %s\n", strerror(errno));
...
void print_config(spw_config *cnf) 
{
        ...
        printf("Link Error Event Task ID: %"PRIu32"\n", cnf->event_id);
...
void check_init_config(spw_config *cnf) 
{
   ...
   if (cnf->event_id != 0) {
           printf("Incorrect initial event id! Expected: %i Got: %"PRIu32"\n", 0, cnf->event_id);
           exit(0);
   }
}
