...
%>\
%if not in_app:
include ${os.path.join(os.path.relpath(xky.ROOT_DIRECTORY, output_dir), 'Makefile.inc')} \
%else:
...
<%
    ...
    import xky
%>\
include ${os.path.join(os.path.relpath(xky.INSTALL_DIRECTORY, output_dir), 'rtems.inc')}\
</%def>
...
#
# Makefile : ${purpose}
# Generated by XKY Configurator Tool v${configurator.__version__}
#\
</%def>
...
-B$(XKY_LIBS)/${libname.lower()}/${'\\' if i < len(libraries) - 1 else ''}
% endfor \
</%def>
...
<%
    ...
    include_dirs = map(lambda x: os.path.join(xky.INSTALL_LIBS_DIRECTORY, x), include_dirs)
