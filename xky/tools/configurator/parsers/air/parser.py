...
#  @author pfnf
#  @brief AIR XML parser base on a653 XML parser
...
##
#
class airParser(a653parser):

    ##
    # @brief air parser constructor
    ...
    def parse_module_extensions(self, xml_node):
        ...
        self.logger.information(1, "AIR (Module Configuration) - Ticks per Second: {0}; Core Count: {1}",
    ...
    def parse_partition_extensions(self, xml, partition):
        ...
        partition.permissions = [ air.PERMISSION_GLOBAL_TIME ]
        ...
        if len(xml_cache_node) > 0 and xml_cache_node[0].text is not None:
            initial_cache = xml_cache_node[0].parse_text(VALID_AIR_CACHE, self.logger, PARTITION_EXTENSION_CACHE_NONE)
