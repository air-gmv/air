...
void entry_func() {
    ...
    xky_syscall_get_schedule_id("schedA", &sched_a_id);
    xky_syscall_get_schedule_id("schedB", &sched_b_id);
    while (1) {

	xky_syscall_get_schedule_status(&status);
	...
	if(3 == i) {
            ...
            xky_syscall_set_schedule(sched_id);
