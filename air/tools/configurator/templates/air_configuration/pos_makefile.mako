<%
    ...
    build_dir = path.relpath(path.join(air.INSTALL_POS_DIRECTORY, name), path.join(output_dir))

    library = path.join(build_dir, '{0}.a'.format(name))
    target_sources = [path.relpath(s, output_dir) for s in sources]
    target_headers = [path.relpath(h, output_dir) for h in headers]
    public_headers = [path.relpath(h, output_dir) for h in library_headers]
%>\
<%namespace name="template" file="/makefile.mako"/>\
${template.FileHeader("POS {0} - {1}".format(name, description))}
