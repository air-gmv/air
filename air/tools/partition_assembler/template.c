...
void ouput_program_segment_data(FILE *fd, uint32_t idx, uint32_t *addr, uint32_t *sizes, uint32_t *offsets,
                                uint32_t size)
{
...
void generate_output(p_data *p_elfs, uint32_t p_count)
{
    ...
    for (i = 0; i < p_count; ++i)
    {
        ...
        fprintf(fd, "extern air_u8_t usr_partition%i_data[%i];\n", i, (int)(p_elfs[i].c_size));
    }
    for (i = 0; i < p_count; ++i)
    {

        fprintf(fd, "extern pmk_segment_t usr_partition%i_segments[%i];\n", i, p_elfs[i].count);
    }
    fprintf(fd, "\n");
    ...
    for (i = 0; i < p_count; ++i)
    {
        fprintf(fd,
                ...
                "    }%s\n",
                p_elfs[i].entry, p_elfs[i].count, i, i, i == p_count - 1 ? "" : ",");
    }
    fprintf(fd, "};\n\n");
    ...
    for (i = 0; i < p_count; ++i)
    {
        ...
        ouput_program_segment_data(fd, i, p_elfs[i].addrs, p_elfs[i].sizes, p_elfs[i].offsets, p_elfs[i].count);
    }

    for (i = 0; i < p_count; ++i)
    {

        output_partition_data(fd, i, p_elfs[i].segments, p_elfs[i].c_size);
    }
