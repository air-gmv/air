...
void test(PARTITION_ID_TYPE self_id)
{
    ...
    while (1)
    {
        ...
        READ_SAMPLING_MESSAGE_CONDITIONAL(RECV_PORT, REF_TIME_STAMP, (MESSAGE_ADDR_TYPE)message, &SIZE, &TIME_STAMP,
                                          &rc);
        if (INVALID_PARAM == rc)
        {
            (void)printf("READ_SAMPLING_MESSAGE_CONDITIONAL error %d\n", rc);
        }
        ...
        {
            (void)printf("Timestamp not yet reached\n");
        }
        else if (TIME_STAMP > REF_TIME_STAMP)
        {
            (void)printf("Message: %s, timestamp=%ldms\n", message, (long int)(TIME_STAMP / 1000000));
        }
        else
        {
            (void)printf("Unexpected READ_SAMPLING_MESSAGE_CONDITIONAL behavior\n");
        }
