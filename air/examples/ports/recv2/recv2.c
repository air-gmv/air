...
void test(uintptr_t self_id)
{
    ...
    while (1)
    {
        (void)printf("Partition %d receiving message..\n", self_id);

        GET_SAMPLING_PORT_CURRENT_STATUS(RECV_PORT2, &STATUS, &rc);
        if (NO_ERROR != rc)
        {
            (void)printf("GET_SAMPLING_PORT_CURRENT_STATUS error %d\n", rc);
        }
        ...
        case NEW_MESSAGE:
            ...
            if (NO_ERROR != rc)
            {
                (void)printf("READ_UPDATED_SAMPLING_MESSAGE error %d\n", rc);
            }

            (void)printf("NEW_MSG: %s\n", message);
        ...
        case CONSUMED_MESSAGE:
            ...
            if (NO_ERROR != rc)
            {
                (void)printf("READ_SAMPLING_MESSAGE error %d\n", rc);
            }
        ...
        case EMPTY_PORT:
            message[0] = '\0';
            (void)printf("Empty sampling port\n");

            break;

        default:
            (void)printf("Error in STATUS.UPDATED value\n");
        ...
        if (rc == NO_ERROR)
        {
            (void)printf("Received Partition Queue message %d: %s\n", self_id, message);
        }
        else
        {
            (void)printf("Error in Receiving Queue Message - %d\n", rc);
        }
