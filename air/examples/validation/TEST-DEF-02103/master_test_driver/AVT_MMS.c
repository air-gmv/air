...
BOOLEAN isPart3InSched( void ) {
	...
	extern BOOLEAN Part3InSched[];
	int CurrentSchedId = -1;

	/*Get Current Schedule ID in order
	to verify te presence of Partition 3 */
	GET_MODULE_SCHEDULE_STATUS(
		&SchedStatus,
		&RetCode);
	CurrentSchedId = (int)(SchedStatus.CURRENT_SCHEDULE);

	#ifdef DEBUG
	printf("Part3InSched: %d, ID=%d\n", 
		Part3InSched[(int)CurrentSchedId - 1],
		(int)CurrentSchedId);
	#endif

	if ( Part3InSched[(int)CurrentSchedId - 1] ) {
		return TRUE;
	}
...
void CheckSetModuleSchedule ( 
	...
	while ( Test_Not_Over ) {
		...
		if (!ErrorOccurred) {
			...
			CycleCounter++;
			if (CycleCounter > MAX_NUM_CYCLES) {
				Test_Not_Over = FALSE;
		...
		if (Test_Not_Over) {
			if (ErrorOccurred) {
				...
				&& (CycleCounter == MAX_NUM_CYCLES)
					) {
				Test_Not_Over = FALSE;
				SetNoErrorBits(ReturnStruct, StackIndex);
