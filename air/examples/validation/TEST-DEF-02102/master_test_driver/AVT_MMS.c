...
BOOLEAN CheckMessageOrder( int count ) {
	...
	extern APEX_LONG_INTEGER Correct_Order[12];
...
void CheckSetModuleSchedule ( 
	...
	while ( Test_Not_Over ) {
		...
		if (RetCode == NO_ERROR) {
			...
			/*---------------------------------------
			printf("\n\n\n\n");
			APEX_LONG_INTEGER *fMessage = (APEX_LONG_INTEGER *)Message_Addr;
			printf("%ld %ld %ld %d %d\n", 
				(long)fMessage[0], (long)fMessage[1],
				(long)fMessage[2], (long)fMessage[3],
				(long)fMessage[4], (long)fMessage[5]);
			---------------------------------------*/
