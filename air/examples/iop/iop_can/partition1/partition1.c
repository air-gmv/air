...
void grcan_send_msg(PARTITION_ID_TYPE self_id)
{
    ...
    while (1)
    {
        ...
        WRITE_SAMPLING_MESSAGE(SEND_PORT, (MESSAGE_ADDR_TYPE)sample1, sizeof(sample1), &rc);

        pprintf("Sending %dbytes: 0xBEEFDEAD\n", sizeof(sample2));
        WRITE_SAMPLING_MESSAGE(SEND_PORT2, (MESSAGE_ADDR_TYPE)sample2, sizeof(sample2), &rc);
...
void grcan_receive_msg(PARTITION_ID_TYPE self_id)
{
    ...
    while (1)
    {
        READ_SAMPLING_MESSAGE(RECEIVE_PORT, (MESSAGE_ADDR_TYPE)recv_msg, &msg_len, &validity, &rc);
        if (msg_len > 0)
        {
            ...
            (void)memset(recv_msg, 0, msg_len);
        }

        READ_SAMPLING_MESSAGE(RECEIVE_PORT2, (MESSAGE_ADDR_TYPE)recv_msg, &msg_len, &validity, &rc);
        if (msg_len > 0)
        {
            pprintf("Partition%d received a message with length %d:\n", self_id, msg_len);
            for (int i = 0; i < msg_len; i++)
