...
#define check_dispatch_disable_level(_expect)                                                                          \
    do                                                                                                                 \
    {                                                                                                                  \
        if (((_expect) != 0) != -1 && (((!_Thread_Dispatch_is_enabled()) == false && (_expect) != 0) ||                \
...
#define check_if_allocator_mutex_is_not_owned()                                                                        \
    do                                                                                                                 \
    {                                                                                                                  \
        if ((_RTEMS_Allocator_is_owner()) != 0)                                                                        \
        {                                                                                                              \
 ...
 */
#define directive_failed(_dirstat, _failmsg) fatal_directive_status(_dirstat, RTEMS_SUCCESSFUL, _failmsg)

#define directive_failed_with_level(_dirstat, _failmsg, _level)                                                        \
    fatal_directive_status_with_level(_dirstat, RTEMS_SUCCESSFUL, _failmsg, _level)

#define fatal_directive_status(_stat, _desired, _msg) fatal_directive_status_with_level(_stat, _desired, _msg, 0)
...
#define fatal_directive_check_status_only(_stat, _desired, _msg)                                                       \
    ...
    {                                                                                                                  \
        if ((_stat) != (_desired))                                                                                     \
        {                                                                                                              \
            printf("\n%s FAILED -- expected (%s) got (%s)\n", (_msg), rtems_status_text(_desired),                     \
...
#define posix_service_failed(_dirstat, _failmsg) fatal_posix_service_status(_dirstat, 0, _failmsg)
...
#define fatal_posix_service_status_errno(_stat, _desired, _msg)                                                        \
    ...
    {                                                                                                                  \
        ...
        check_if_allocator_mutex_is_not_owned();                                                                       \
        printf("\n%s FAILED -- expected (%d - %s) got (%ld %d - %s)\n", (_msg), _desired, strerror(_desired), statx,   \
...
#define fatal_posix_service_status_with_level(_stat, _desired, _msg, _level)                                           \
    ...
    {                                                                                                                  \
        ...
        if ((_stat) != (_desired))                                                                                     \
        {                                                                                                              \
            printf("\n%s FAILED -- expected (%d - %s) got (%d - %s)\n", (_msg), _desired, strerror(_desired), _stat,   \
                   strerror(_stat));                                                                                   \
            printf("\n FAILED -- errno (%d - %s)\n", errno, strerror(errno));                                          \
            rtems_test_exit(_stat);                                                                                    \
        }                                                                                                              \
...
#define int_service_failed(_dirstat, _failmsg) fatal_int_service_status(_dirstat, RTEMS_SUCCESSFUL, _failmsg)

#define int_service_failed_with_level(_dirstat, _failmsg, _level)                                                      \
    fatal_int_service_status_with_level(_dirstat, RTEMS_SUCCESSFUL, _failmsg, _level)

#define fatal_int_service_status(_stat, _desired, _msg) fatal_int_service_status_with_level(_stat, _desired, _msg, 0)
...
#define sprint_time(_str, _s1, _tb, _s2)                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
        sprintf((_str), "%s%02d:%02d:%02d   %02d/%02d/%04d%s", _s1, (_tb)->hour, (_tb)->minute, (_tb)->second,         \
...
#define print_time(_s1, _tb, _s2)                                                                                      \
    do                                                                                                                 \
    {                                                                                                                  \
        printf("%s%02" PRIu32 ":%02" PRIu32 ":%02" PRIu32 "   %02" PRIu32 "/%02" PRIu32 "/%04" PRIu32 "%s", _s1,       \
               (_tb)->hour, (_tb)->minute, (_tb)->second, (_tb)->month, (_tb)->day, (_tb)->year, _s2);                 \
    } while (0)
...
#define task_number(tid)                                                                                               \
    (rtems_object_id_get_index(tid) - rtems_configuration_get_rtems_api_configuration()->number_of_initialization_tasks)
...
#define COMPILER_DIAGNOSTIC_SETTINGS_DISABLE_NONNULL _Pragma("GCC diagnostic ignored \"-Wnonnull\"")
#else
