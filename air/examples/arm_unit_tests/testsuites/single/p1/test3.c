...
void test3()
{
  ...
  {
    pprintf( " FAIL3 : task allocation size must be greater than 4.\n");
    exit( 1 );
  }
  ...
  while (task_count < MAX_TASKS)
  {
    ...
    pprintf(" TEST3 : creating task '%c%c%c%c', ", c1, c2, c3, c4);
  ...
  {
    pprintf( " FAIL3 : not enough tasks created -\n"
  ...
  for (block = 0; block < TASK_ALLOCATION_SIZE; block++)
  {
         ...
         remove_task++)
    {
      ...
      {
        pprintf( " FAIL3 : remove task has a 0 id -\n"
                "         task number = %" PRIi32 "\n",
                remove_task);
        exit( 1 );
      }

      pprintf(" TEST3 : remove, signal task %08" PRIxrtems_id ", ", task_id[remove_task]);
