...
void test2()
{
  ...
  while (task_count < ((TASK_ALLOCATION_SIZE * 5) - TASK_INDEX_OFFSET))
  {
    ...
    pprintf(" TEST2 : creating task '%c%c%c%c', ", c1, c2, c3, c4);
    ...
    pprintf("number = %3" PRIi32 ", id = %08" PRIxrtems_id ", starting, ", task_count, task_id[task_count]);
    fflush(stdout);
  ...
  if (task_count != ((TASK_ALLOCATION_SIZE * 5) - TASK_INDEX_OFFSET)) {
    pprintf( " FAIL2 : not enough tasks created -\n"
  ...
  for (block = 1; block < 4; block += 2)
  {
         ...
         remove_task++)
    {
      ...
      {
        pprintf( " FAIL2 : remove task has a 0 id -\n"
                "         task number = %" PRIi32 "\n",
                remove_task);
        destroy_all_tasks("TEST2");
        exit( 1 );
      ...
      pprintf(" TEST2 : block %" PRIi32 " remove, signal task %08"
  ...
  for (task = 0; task < (TASK_ALLOCATION_SIZE * 2); task++)
  {
    ...
    {
      pprintf( " FAIL2 : no free task id slot.\n");
      destroy_all_tasks("TEST2");
      exit( 1 );
    }
    ...
    if (status_code_bad(result))
    {
      pprintf( " FAIL2 : re-creating a task -\n"
              "         task number = %" PRIi32 "\n",
              id_slot);
      destroy_all_tasks("TEST2");
      exit( 1 );
    ...
    pprintf("number = %3" PRIi32 ", id = %08" PRIxrtems_id ", starting, ",
    ...
    if (status_code_bad(result))
    {
      pprintf( " FAIL : re-starting a task -\n"
              "        task number = %" PRIi32 "\n",
              id_slot);
      destroy_all_tasks("TEST2");
      exit( 1 );
    ...
    if (remove_task == (TASK_ALLOCATION_SIZE * 2))
    {
      ...
      pprintf( " FAIL2 : could not find the task id in the removed table -\n"
              "         task id = %08" PRIxrtems_id "\n",
              task_id[id_slot]);
      exit( 1 );
    }
