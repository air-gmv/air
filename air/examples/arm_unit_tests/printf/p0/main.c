...
void entry_point() {
	...
	printf("P0:: us_per_tick = %lluus\n", us_per_tick);

	air_i32_t ms_per_tick = (air_u32_t)us_per_tick/1000;
	printf("P0:: ms_per_tick = %ldms\n", ms_per_tick);

    int p = 0;

	for(int i = 0; i < 0x100000; ++i);

	printf("P0:: Sanity check. I am here at partition %d. ms/tick = %ld, *2 %ld, +1 %ld, /2 %ld \n",
	...
	printf("P0:: I am here at tick(time): %llu(%llums)\n", i_tick, i_time);

	for(int i = 0; i < 0x100000; ++i);
    tick = air_syscall_get_elapsed_ticks();
    time = (air_u64_t)(tick * ms_per_tick);
	printf("P0:: I am here at tick(time) after d_ticks: %llu(%llums) %llu\n", tick, time, tick - i_tick);
