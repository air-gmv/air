...
# mine avec pompe
# Emmanuel Grolleau
# Derniere modification 28/11/2012
##############################################################
...
if {[catch {socket -server acceptcx $PORT}]} {
	tk_messageBox -icon error -title "Erreur d'initialisation" -message "Erreur lors de la création du serveur TCP\nVérifiez qu'un autre simulateur ne tourne pas déjà!!!"
	exit
}
...
# Fait evoluer le niveau de l'eau avec la pompe ouverte (si v=-1)
# ou fermee (si v=1)
  global minepump
  if {$v>0} {
