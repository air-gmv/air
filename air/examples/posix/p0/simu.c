...
BYTE ReadMS(void)
{
    ...
    b = fgetc(sockin);
#else
    /* If there is no GUI, we simply emulate the methane sensor. The methan
       gets up and down */
...
void CmdPump(BYTE cmd)
{
    ...
    (void)fprintf(sockout, "WL = %03d\t", WaterLevel_Value);

    (void)fprintf(sockout, "Pump %d\t", CmdPump_Value);
