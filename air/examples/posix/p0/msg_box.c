...
msg_box msg_box_init(const unsigned elt_size)
{
    ...
    CHECK_NZ(pthread_mutex_init(&((*mbox).mutex), 0));
    CHECK_NZ(pthread_cond_init(&((*mbox).not_empty), 0));
...
int msg_box_receive(msg_box mbox, char *buf)
{
    ...
    CHECK_NZ(pthread_mutex_lock(&(*mbox).mutex)); /* Enter critical section */
    ...
    CHECK_NZ(pthread_mutex_unlock(&(*mbox).mutex)); /* Leave critical section */
...
int msg_box_send(msg_box mbox, const char *buf)
{
    ...
    CHECK_NZ(pthread_cond_signal(&(*mbox).not_empty));
