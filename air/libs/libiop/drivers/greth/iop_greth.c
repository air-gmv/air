...
static int greth_initialize_hardware(iop_eth_device_t *device)
{
    ...
    if ((phyctrl >> 12) & 1)
    {
		...
		if ( rtems_clock_get_tod_timeval(&tstart) == AIR_NOT_AVAILABLE){ // use for RTEMS 5
...
static int greth_hw_receive(greth_softc_t *sc, iop_wrapper_t *wrapper, iop_eth_device_t *device)
{
    ...
    while (!((status = sc->rxdesc[sc->rx_ptr].ctrl) & GRETH_RXD_ENABLE))
    {
        ...
        if (error == 0)
        {
            ...
            /*get packet header*/
            (void)memcpy(&frag->header, sc->rx_iop_buffer[sc->rx_ptr]->v_addr, sizeof(eth_header_t));
            ...
            if ((valid = greth_validate_packet(device, &frag->header.eth_header)) == 0)
            {
                ...
                if (pack_seq == 0)
                {
                    ...
                    (void)memcpy(wrapper->buffer->v_addr, sc->rx_iop_buffer[sc->rx_ptr]->v_addr, len);
            ...
            else
            {
                ...
                if (valid == 1)
                {
                    ...
                    /*set descriptor info to wrapper*/
                    (void)memcpy(arp_wrapper->buffer->v_addr, sc->rx_iop_buffer[sc->rx_ptr]->v_addr, len);
...
static int greth_hw_send(greth_softc_t *sc, iop_wrapper_t *wrapper)
{
    ...
    (void)memcpy(sc->tx_iop_buffer[sc->tx_ptr]->v_addr, &frag->header, frag->header_size);

    /*Copy payload to descriptor*/
    (void)memcpy(sc->tx_iop_buffer[sc->tx_ptr]->v_addr + frag->header_size, frag->payload, frag->payload_size);
...
uint32_t greth_initialize(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    (void)memset(&apbgreth, 0, sizeof(amba_apb_dev_t));
...
uint32_t greth_reset(iop_device_driver_t *iop_dev)
{
    ...
    /*Initialize the eth core*/
    (void)greth_initialize_hardware(device);
