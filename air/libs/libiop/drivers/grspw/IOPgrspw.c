...
#define SPACEWIRE_DBG2(fmt)                                                                                            \
    ...
    {                                                                                                                  \
        {                                                                                                              \
            iop_debug(" : %03d @ %18s()]:" fmt, __LINE__, __FUNCTION__);                                               \
        }                                                                                                              \
    } while (0)
...
{
    unsigned int tmp;
    __asm__ __volatile__(" lda [%1]1, %0 " : "=r"(tmp) : "r"(addr));
    return tmp;
}
...
{
    unsigned int tmp;
    __asm__ __volatile__(" lduba [%1]1, %0 " : "=r"(tmp) : "r"(addr));
    return tmp;
}
#endif
...
void spw_hw_handle_errors(SPW_DEV *pDev)
{
    ...
    SPW_STATUS_WRITE(pDev, SPW_STATUS_CE | SPW_STATUS_ER | SPW_STATUS_DE | SPW_STATUS_PE | SPW_STATUS_WE |
...
uint32_t spw_initialize(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    else if ((amba_get_apb_slave(amba_bus, VENDOR_GAISLER, GAISLER_SPW2, cores_init, &dev)) != 0)
    ...
    else if ((amba_get_apb_slave(amba_bus, VENDOR_GAISLER, GAISLER_SPW2_DMA, cores_init, &dev)) != 0)
    ...
    setup_iop_buffers(pDev->iop_buffers, pDev->iop_buffers_storage, 2 * pDev->txbufcnt + pDev->rxbufcnt);
...
uint32_t spw_read(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    if ((DEBUG_SPACEWIRE_FLAGS & DBGSPW_DUMP) != 0)
    {
        ...
        for (k = 0; k < count; k++)
        {
            ...
            iop_debug("%.0x(%x) ", *((uint8_t *)iop_buffer->v_addr + k) & 0xff,
                      isprint(*((uint8_t *)iop_buffer->v_addr + k) & 0xff) ? *((uint8_t *)iop_buffer->v_addr + k) & 0xff
                                                                           : ' ');
        }
        iop_debug("\n");
    }
...
uint32_t spw_write(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    if ((iop_buffer->payload_size > pDev->txdbufsize) || (iop_buffer->payload_size < 1) || (iop_buffer->v_addr == NULL))
    {
        ...
        iop_debug("len: %ld, data: %s, bufsize: %ld\n", iop_buffer->payload_size, (char *)iop_buffer->v_addr,
                  pDev->txdbufsize);
        return AIR_INVALID_PARAM;
    }
    ...
    int data_sent = 0;
    while (
        (data_sent = spw_hw_send(pDev, iop_buffer->header_size - iop_buffer->header_off,
                                 ((uint8_t *)iop_buffer->v_addr + iop_buffer->header_off), iop_buffer->payload_size + 1,
                                 ((uint8_t *)iop_buffer->v_addr + iop_buffer->payload_off - 1))) == 0)
    {
...
//                         int)ioarg->buffer); if ((unsigned int)ioarg->buffer > 255) {
//                                 return AIR_INVALID_PARAM;
//                         }
...
//                         int)ioarg->buffer); if ( pDev->core_ver > 1 ){
...
//                         int)ioarg->buffer); if ((unsigned int)ioarg->buffer > 1) {
//                                 return AIR_INVALID_PARAM;
//                         }
...
//                         int)ioarg->buffer); if (!pDev->config.is_rmap) {
...
//                                   return AIR_INVALID_PARAM;
//                           }
//                           SPW_WRITE(&pDev->regs->timer, (SPW_READ(&pDev->regs->timer) & 0xFFFFF000) | ((unsigned
//                           int)ioarg->buffer & 0xFFF)); if ((SPW_READ(&pDev->regs->timer) & 0xFFF) != (unsigned
//                           int)ioarg->buffer) {
//                                   return AIR_DEVICE_ERROR;
//                           }
...
//                                   return AIR_INVALID_PARAM;
//                           }
//                           SPW_WRITE(&pDev->regs->timer, (SPW_READ(&pDev->regs->timer) & 0xFFC00FFF) | (((unsigned
//                           int)ioarg->buffer & 0x3FF) << 12)); if (((SPW_READ(&pDev->regs->timer) >> 12) & 0x3FF) !=
//                           (unsigned int)ioarg->buffer) {
//                                   return AIR_DEVICE_ERROR;
//                           }
...
//                                 return AIR_INVALID_PARAM;
//                         }
//                         SPW_CTRL_WRITE(pDev, (SPW_STATUS_READ(pDev) & 0xFFFEFFFF) | ((unsigned int)ioarg->buffer <<
...
//                                 return AIR_INVALID_PARAM;
//                         }
//                         SPW_CTRL_WRITE(pDev, (SPW_STATUS_READ(pDev) & 0xFFFDFFFF) | ((unsigned int)ioarg->buffer <<
...
//                         int)ioarg->buffer); SPACEWIRE_DBGC(DBGSPW_IOCTRL, "CTRL REG: %x\n", SPW_CTRL_READ(pDev)); if
//                         ((unsigned int)ioarg->buffer > 1) {
//                                 return AIR_INVALID_PARAM;
//                         }
//                         SPW_CTRL_WRITE(pDev, (SPW_CTRL_READ(pDev) & 0xFFFFFDF7) | ((unsigned int)ioarg->buffer << 9)
//                         | ((unsigned int)ioarg->buffer << 3)); SPACEWIRE_DBGC(DBGSPW_IOCTRL, "CTRL REG: %x\n",
//                         SPW_CTRL_READ(pDev)); if (((SPW_CTRL_READ(pDev) >> 9) & 1) != (unsigned int)ioarg->buffer) {
//                                 return AIR_DEVICE_ERROR;
//                         }
...
//                         int)ioarg->buffer); pDev->config.event_id = (unsigned int)ioarg->buffer;
...
//                         int)((SPW_STATUS_READ(pDev) >> 21) & 0x7));
...
//                         major, minor,
//                                        (unsigned int)args->hlen, (int)args->hdr,(unsigned int)args->dlen,
//                                        (int)args->data);
//
//                         /*Check For errors*/
//						spw_hw_handle_errors(pDev);
...
//                             ((args->hdr == NULL) && (args->hlen != 0)) || ((args->data == NULL) && (args->dlen !=
//                             0))) {
//                                 return AIR_INVALID_PARAM;
//                         }
...
int spw_hw_init(SPW_DEV *pDev)
{
    ...
    SPACEWIRE_DBG("pDev->rx : 0x%x = (physical)0x%x\n   pDev->tx : 0x%x = (physical)0x%x\n", pDev->rx,
                  (unsigned int)air_syscall_get_physical_addr((air_uptr_t)pDev->rx), pDev->tx,
...
int spw_hw_startup(SPW_DEV *pDev, int timeout)
{
    ...
    SPW_WRITE(&pDev->regs->status, (SPW_STATUS_TO | SPW_STATUS_CE | SPW_STATUS_ER | SPW_STATUS_DE | SPW_STATUS_PE |
                                    SPW_STATUS_WE | SPW_STATUS_IA | SPW_STATUS_EE));

    /*Wait for the SPW link to be ready*/
    if ((spw_hw_waitlink(pDev, timeout)) != 0)
    {
    ...
    /*Enable RX*/
    SPW_WRITE(&pDev->regs->dma0ctrl,
              (dmactrl & SPW_PREPAREMASK_RX) | SPW_DMACTRL_RD | SPW_DMACTRL_RXEN | SPW_DMACTRL_NS);
...
int spw_hw_receive(SPW_DEV *pDev, uint8_t *b, int c)
{
     ...
     */
    if ((pDev->config.promiscuous) != 0)
    {

        /* make sure address and prot can be read in promiscuous mode */
        dump_start_len = 0;
    }
    else if ((pDev->config.rm_prot_id) != 0)
    ...
    if (!((ctrl & SPW_RXBD_ERROR) || (pDev->config.check_rmap_err && (ctrl & SPW_RXBD_RMAPERROR))))
    {
        ...
        if (rxlen > 0)
        {
            if ((CPU_SPARC_HAS_SNOOPING) != 0)
            {

                /*Copy received data to user's buffer*/
                (void)memcpy(b, rxb + pDev->rxbufcur, rxlen);
            }
            else
            {
...
// } else {
// /*Use user's configurations for this core*/
// size = size + (user_config[i]->txdbufsize+user_config[i]->txhbufsize)*user_config[i]->txdesccnt +
// user_config[i]->rxbufsize*user_config[i]->rxdesccnt;
// }
// }
