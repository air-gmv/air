...
void *find_shortcut(iop_chain_control *rtaddress_chain, milstd_header_t *hdr, unsigned int size)
{
    ...
    if (!iop_chain_is_empty(rtaddress_chain))
    {
        ...
        while (shortcut != (write_cmd_shortcut_t *)iop_chain_tail(rtaddress_chain))
        {
            ...
            if ((shortcut->subaddr == hdr->address) && (shortcut->wcmc == (size / 2)))
            {
...
void create_write_cmd_shortcut(grb_priv *priv, iop_chain_control *rtaddress_chain, bc_command_t *ul, void *desc)
{
    ...
    iop_chain_append(rtaddress_chain, &shortcut->node);
}
...
static inline uint32_t create_descriptor_word0(int exttrig, int exclusive, int retrymode, int nretry, int gap, int time)
{
    uint32_t word = 0;

    /* external trigger, exclusive time slot, retry mode, number of retries */
    word = (((exttrig & 0x1) << 30) | ((exclusive & 0x1) << 29) | ((retrymode & 0x3) << 23) | ((nretry & 0x7) << 20));
        ...
        if (bc_verify_command_status(desc->status) == AIR_SUCCESSFUL)
        {
            ...
            if ((subaddress1 == 0) || (subaddress1 == 31))
            {
                ...
                if ((wcmc == 16) || (wcmc == 18) || (wcmc == 19))
                {
                    ...
                    (void)memcpy(get_payload(buf), (void *)get_virtual_addr(priv, desc->dptr), 2);
            ...
            else
            {
                ...
                /* copy received data to user buffer */
                (void)memcpy(get_payload(buf), (void *)get_virtual_addr(priv, desc->dptr), wcmc * 2);
...
uint32_t gr1553bc_write(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    desc = (struct gr1553bc_bd_tr *)find_shortcut(&priv->shortcut[(hdr->desc & 0x1f)], hdr,
                                                  get_payload_size(wrapper->buffer));
    ...
    if (desc != NULL)
    {
        ...
        /* lets copy the data */
        memcpy((void *)get_virtual_addr(priv, desc->dptr), (void *)get_payload(wrapper->buffer),
               get_payload_size(wrapper->buffer));
...
air_status_code_e gr1553bc_erase_async_data(grb_priv *priv)
{
    ...
    (void)memset(priv->async, 0, (priv->a_cl_size - 1) * 16);

    (void)memset(priv->async_buf_mem_start, 0, (priv->a_cl_size - 1) * 2 * 32);
...
air_status_code_e gr1553bc_add_async_data(grb_priv *priv, uint8_t *data, milstd_header_t *hdr, uint32_t size)
{
    ...
    /* lets copy the data */
    (void)memcpy(buffer, (void *)data, size);
...
static void translate_command(grb_priv *priv, unsigned int offset, unsigned int *data_offset)
{
    ...
    else if (((user_list->ccw) & END_BIT) != 0)
    ...
    { /*< Transfer */
        ...
        word0 = create_descriptor_word0(wtrig, excl, priv->user_config->retry_mode, nret,
                                        priv->user_config->msg_timeout, user_list->time_slot);
        ...
        if (bc_to_rt == 1)
        {
            ...
            if (!(user_list->ccw & DUMMY_BIT))
                create_write_cmd_shortcut(priv, &priv->shortcut[(user_list->rtaddr[0] & 0x1f)], user_list,
                                          (void *)desc);
        }
