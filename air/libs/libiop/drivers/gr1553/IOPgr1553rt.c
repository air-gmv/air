...
static void gr1553rt_init_table(grb_priv *priv)
{
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        if ((((priv->user_config->enabled_subs >> i) & 0x1)) != 0)
        {
            z++;
        }
    }
...
void gr1553rt_device_init(grb_priv *priv)
{
    ...
    priv->regs->rt_cfg =
        GR1553RT_KEY | (priv->user_config->rtaddress << GR1553B_RT_CFG_RTADDR_BIT) | GR1553B_RT_CFG_RTEN;
...
uint32_t gr1553rt_read(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    for (i = 0; ((i < 32) && (read == 0)); i++)
    {
        ...
        while ((((bd->ctrl & 0x80000000) >> 31) == 1) && (read == 0) && (z < buf_per_sub))
        {
            ...
            if ((wc > 0) /*&& (sw == 0)*/)
            {
                ...
                (void)memcpy(get_payload(wrapper->buffer), (void *)bd->dptr, wc * 2);
...
uint32_t gr1553rt_write(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    /* copy data buffer */
    (void)memcpy((void *)bd->dptr, get_payload(wrapper->buffer), get_payload_size(wrapper->buffer));
