...
air_u32_t iop_xeth_read(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    if (xs->rx_bd[xs->rx_ptr].addr & 0x1)
    {
        ...
        (void)memcpy(&frag->header, xs->rx_iop_buffer[xs->rx_ptr]->v_addr, sizeof(eth_header_t));
        ...
        if ((valid = validate_packet(device, &frag->header.eth_header)) == 0)
        {
            ...
            unsigned int pack_seq =
                (((unsigned int)frag->header.eth_header.ipoffset[0] & 0x1f) << 8) + frag->header.eth_header.ipoffset[1];
            ...
            if (pack_seq == 0)
            {
                ...
                (void)memcpy(wrapper->buffer->v_addr, xs->rx_iop_buffer[xs->rx_ptr]->v_addr, len);
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        memcpy(frag->payload, xs->rx_iop_buffer[xs->rx_ptr]->v_addr + frag->header_size,
                               frag->payload_size);
                        wrapper->buffer->payload_size += frag->payload_size;
                        iop_chain_append(&wrapper->fragment_queue, &frag->node);
                    }
        ...
        else
        {
            ...
            if (valid == 1)
            {
                ...
                (void)memcpy(arp_wrapper->buffer->v_addr, xs->rx_iop_buffer[xs->rx_ptr]->v_addr, len);
...
air_u32_t iop_xeth_write(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    (void)memcpy(xs->tx_iop_buffer[xs->tx_ptr]->v_addr, &frag->header, frag->header_size);

    (void)memcpy(xs->tx_iop_buffer[xs->tx_ptr]->v_addr + frag->header_size, frag->payload, frag->payload_size);
