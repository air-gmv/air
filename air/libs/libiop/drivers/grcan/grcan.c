...
static int grcan_hw_read_try(grcan_priv *pDev, struct grcan_regs *regs, CANMsg *buffer, int max);

static int grcan_hw_write_try(grcan_priv *pDev, struct grcan_regs *regs, CANMsg *buffer, int count);

static void grcan_hw_config(struct grcan_regs *regs, struct grcan_config *conf);

static void grcan_hw_accept(struct grcan_regs *regs, struct grcan_filter *afilter);
...
{
    unsigned char tmp;
    __asm__(" lduba [%1]1, %0 " : "=r"(tmp) : "r"(address));
    return tmp;
}
#else
...
{
    unsigned int tmp;
    __asm__(" lda [%1]1, %0 " : "=r"(tmp) : "r"(address));
    return tmp;
}
#endif
...
static air_status_code_e grcan_hw_start(grcan_priv *pDev)
{
    ...
    /* Setup receiver */
    pDev->regs->rx0addr = (unsigned int)air_syscall_get_physical_addr((air_uptr_t)pDev->rx);
    pDev->regs->rx0size = pDev->rxbuf_size;

    /* Setup Transmitter */
    pDev->regs->tx0addr = (unsigned int)air_syscall_get_physical_addr((air_uptr_t)pDev->tx);
...
static void grcan_hw_config(struct grcan_regs *regs, struct grcan_config *conf)
{
    ...
    if ((conf->selection.selection) != 0)
    {
        config |= GRCAN_CFG_SELECTION;
    }

    if ((conf->selection.enable0) != 0)
    {
        config |= GRCAN_CFG_ENABLE0;
    }

    if ((conf->selection.enable1) != 0)
    {
...
static unsigned int grcan_hw_rxavail(unsigned int rp, unsigned int wp, unsigned int size)
{
    if (rp == wp)
    {
...
static unsigned int grcan_hw_txspace(unsigned int rp, unsigned int wp, unsigned int size)
{
    unsigned int left;
    iop_debug("ARGS\nwp\trp\tsize\n%d\t%d\t%d \n", wp, rp, size);
    if (rp == wp)
...
static int grcan_calc_timing(unsigned int baud,    /* The requested BAUD to calculate timing for */
                             unsigned int core_hz, /* Frequency in Hz of GRCAN Core */
                             unsigned int sampl_pt, struct grcan_timing *timing /* result is placed here */
)
{
    ...
    for (tseg = (MIN_TSEG1 + MIN_TSEG2 + 2) * 2; tseg <= (MAX_TSEG2 + MAX_TSEG1 + 2) * 2 + 1; tseg++)
    {
        brp = core_hz / ((1 + tseg / 2) * baud) + tseg % 2;
        if ((brp <= 0) || ((brp > 256 * 1) && (brp <= 256 * 2) && (brp & 0x1)) ||
            ((brp > 256 * 2) && (brp <= 256 * 4) && (brp & 0x3)) ||
            ((brp > 256 * 4) && (brp <= 256 * 8) && (brp & 0x7)) || (brp > 256 * 8))
            continue;
...
static int grcan_hw_write_try(grcan_priv *pDev, struct grcan_regs *regs, CANMsg *buffer, int count)
{
    ...
    while (space_left > 0)
    {
        ...
        dest = ((unsigned int)dest >= txmax) ? (grcan_msg *)addr : dest + 1;
        space_left--;
...
int iop_grcan_device_init(iop_device_driver_t *iop_dev)
{
    ...
    (void)memset(&grcandev, 0, sizeof(amba_apb_dev_t));
...
if((rtems_semaphore_obtain(pDev->dev_sem, RTEMS_WAIT, RTEMS_NO_TIMEOUT) != 0)
    ...
    pDev->tx = (grcan_msg *)(((unsigned int)pDev->_tx + (BUFFER_ALIGNMENT_NEEDS - 1)) & ~(BUFFER_ALIGNMENT_NEEDS - 1));
    pDev->rx =
        (grcan_msg *)((((unsigned int)pDev->_rx + (BUFFER_ALIGNMENT_NEEDS - 1)) & ~(BUFFER_ALIGNMENT_NEEDS - 1)) +
                      pDev->txbuf_size);

    (void)iop_grcan_set_afilter(iop_dev, &(pDev->afilter));

    (void)iop_grcan_set_sfilter(iop_dev, &(pDev->sfilter));

    (void)memset(&pDev->stats, 0, sizeof(struct grcan_stats));
...
uint32_t iop_grcan_open(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    if ((iop_grcan_clr_stats(iop_dev)) != 0)
    {
...
uint32_t iop_grcan_read(iop_device_driver_t *iop_dev, void *arg)
{
    ...
    if (!(pDev->rxblock && pDev->rxcomplete && (count != req_cnt)))
    {
        ...
        if (count > 0)
        {
            ...
            (void)memcpy(get_header(iop_buffer), &header, get_header_size(iop_buffer));

            memcpy(get_payload(iop_buffer), dest.data, get_payload_size(iop_buffer));
