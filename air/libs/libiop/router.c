...
static void send_remote_reply(iop_wrapper_t *wrapper, iop_port_t *port)
{
    ...
    if (port->type == AIR_QUEUING_PORT)
    {
        ...
        air_queuing_port_configuration_t *config = (air_queuing_port_configuration_t *)port->configuration;
    ...
    else if (port->type == AIR_SAMPLING_PORT)
    {
        ...
        air_sampling_port_configuration_t *config = (air_sampling_port_configuration_t *)port->configuration;
    ...
    /* send data */
    air_status_code_e rc = air_syscall_write_port(port->type, port->id, (air_message_ptr_t)get_payload(wrapper->buffer),
                                                  (size_t)size, (void *)NULL);
...
void route_request(iop_logical_device_t *ldev, iop_wrapper_t *wrapper)
{
    ...
    for (i = 0; i < ldev->routes.length; ++i)
    {
        ...
        /* route pointer */
        iop_logical_route_t *route = &((iop_logical_route_t *)ldev->routes.elements)[i];
...
void route_reply(iop_physical_device_t *pdev, iop_wrapper_t *wrapper)
{
    ...
    for (i = 0; i < pdev->routes.length; ++i)
    {
        ...
        if (1 == route->schedule[air_schedule.current_schedule_index])
        {
            ...
            if ((pdev->header_compare(wrapper, route->header)) != 0)
            {
...
void pre_router()
{
    ...
    for (i = 0; i < usr_configuration.logical_devices.length; ++i)
    {
        ...
        /* get logical device */
        iop_logical_device_t *ldev = ((iop_logical_device_t **)usr_configuration.logical_devices.elements)[i];
        ...
        while (!iop_chain_is_empty(&ldev->sendqueue))
        {
            ...
            iop_wrapper_t *req_wrapper = obtain_wrapper(&ldev->sendqueue);
...
void pos_router()
{
    ...
    for (i = 0; i < usr_configuration.physical_devices.length; ++i)
    {
        ...
        /* get physical device */
        iop_physical_device_t *pdev = ((iop_physical_device_t **)usr_configuration.physical_devices.elements)[i];
