...
static void iop_init_queues(void)
{
    ...
    for (i = 0; i < usr_configuration.remote_ports.length; ++i)
    {
        ...
        if (port->type == AIR_QUEUING_PORT)
        {
            ...
            if ((config.qport->max_message_size > size) != 0)
            {
        ...
        else
        {
            ...
            if ((config.sport->max_message_size > size) != 0)
            {
...
static air_status_code_e iop_init_ports()
{
    ...
    for (i = 0; i < usr_configuration.remote_ports.length; ++i)
    {
        ...
        /* create port */
        air_status_code_e p_rc = air_syscall_create_port(port->type, port->name, port->configuration, &port->id);
