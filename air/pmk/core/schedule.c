...
void pmk_scheduler_init(void)
{
    ...
    pmk_barrier_init(&scheduler_ctrl.mtf_barrier, air_shared_area.configured_cores);
...
void pmk_apply_next_schedule(pmk_core_ctrl_t *core)
{
    ...
    if (scheduler_ctrl.curr_schedule != NULL)
    {
        ...
        for (i = 0; i < scheduler_ctrl.curr_schedule->partitions.length; ++i)
        {
            ...
            partition_schedule = &((pmk_partition_schedule_t *)scheduler_ctrl.curr_schedule->partitions.elements)[i];
    ...
    for (i = 0; i < scheduler_ctrl.next_schedule->partitions.length; ++i)
    {
        ...
        partition_schedule = &((pmk_partition_schedule_t *)scheduler_ctrl.next_schedule->partitions.elements)[i];
        partition = partition_schedule->partition;
        ...
        if (1 == partition_schedule->set_schedule || AIR_PERMISSION_SUPERVISOR == partition->permissions)
        {
            ...
            partition_schedule->partition->permissions |= AIR_PERMISSION_SET_SCHEDULE;
        }
        else
        {
    ...
    for (i = 0; i < air_shared_area.configured_cores; ++i)
    {
        ...
        air_shared_area.core[i].schedule = &scheduler_ctrl.next_schedule->cores[i];
    }
