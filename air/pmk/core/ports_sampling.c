...
void pmk_sampling_channel_init(pmk_channel_t *channel)
{
    ...
    pmk_sampling_config_t *config = (pmk_sampling_config_t *)channel->configuration;
    ...
    config->buffer = (pmk_message_slot_t *)pmk_workspace_alloc(sizeof(pmk_message_slot_t) * BUFFER_SIZE);
    ...
    for (i = 0; i < BUFFER_SIZE; ++i)
    {
        ...
        config->buffer[i].message = (air_message_ptr_t)pmk_workspace_alloc(config->max_message_size);
#ifdef PMK_DEBUG
...
air_u32_t pmk_sampling_port_check_configuration(pmk_port_t *port, air_sampling_port_status_t *usr_config)
{
    ...
    pmk_sampling_config_t *port_config = (pmk_sampling_config_t *)port->channel->configuration;
    if (port_config->max_message_size != usr_config->max_message_size ||
        port_config->refresh_period != usr_config->refresh_period || port->direction != usr_config->port_direction)
    {
                                               ...
                                               air_sampling_port_status_t *status)
{
    ...
    pmk_sampling_config_t *config = (pmk_sampling_config_t *)port->channel->configuration;
    ...
    if (((air_shared_area.schedule_ctrl->total_ticks - l_status.time_stamp) > l_status.refresh_period) != 0)
    {
                                         ...
                                         air_sz_t *length, air_sampling_port_status_t *status)
{
    ...
    if (updated != AIR_EMPTY_PORT)
    {
        ...
        if (((air_shared_area.schedule_ctrl->total_ticks - slot->timestamp) > config->refresh_period) != 0)
        {
