...
void pmk_segregation_int(void)
{
    ...
    for (i = 0; i < partition_list->length; ++i)
    {
        ...
        pmk_partition_t *partition = pmk_get_from_list(pmk_partition_t, partition_list, i);
        ...
        cpu_segregation_map_memory(partition->mmu_ctrl, (void *)p_addr, partition->mmap->v_addr, partition->mmap->size,
                                   partition->mmap->p_unit, PMK_MMU_R | PMK_MMU_W | PMK_MMU_E | PMK_MMU_CACHEABLE);
    ...
    for (i = 0; i < shm_areas_list->length; ++i)
    {
        ...
        for (j = 0; j < shm_access_list->length; ++j)
        {
            ...
            pmk_shm_partition_t *shm_partition = pmk_get_from_list(pmk_shm_partition_t, shm_access_list, j);
            ...
            cpu_segregation_map_memory(partition->mmu_ctrl, (void *)p_addr, shm->addr, shm->size, shm->unit,
