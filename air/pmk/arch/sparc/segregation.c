 ...
 */
static air_u32_t sparc_mmu_fill_table(air_u32_t *table, void *p_addr, void *v_addr, air_u32_t n, air_u32_t unit,
                                      air_sz_t size, air_u32_t access)
{
    ...
    while (size > 0 && i < level_entries - 1)
    {
        ...
        if (unit < level_unit)
        {
            ...
            l_consumed = sparc_mmu_fill_table(ltable, p_addr, v_addr, n + 1, unit, size, access);
        }
        else
        {
...
void sparc_map_memory(mmu_context_t *ctrl, void *p_addr, void *v_addr, air_sz_t size, air_sz_t unit,
                      air_u32_t permissions)
{
    ...
    if (size != 0)
    {
...
#ifdef PMK_DEBUG
        printk("      [%p-%p]->[%p-%p] (u:%p, p:0x%03x)\n", v_addr, v_addr + size - 1, p_addr, p_addr + size - 1, unit,
               sparc_permissions);
#endif
        ...
        /* fill tables */
        sparc_mmu_fill_table(ctrl->l1_tables, p_addr, v_addr, 0, unit, size, sparc_permissions);
    }
}
 ...
 */
static air_u32_t sparc_segregation_memcpy(core_context_t *core_ctx, void *dst, void *src, air_sz_t size)
{
...
air_u32_t sparc_copy_to_user(core_context_t *core_ctx, void *dst, void *src, air_sz_t size)
{
...
air_u32_t sparc_copy_from_user(core_context_t *core_ctx, void *dst, void *src, air_sz_t size)
{
    ...
    for (j = 0; j < 3; ++j)
    {
        ...
        if (et == 2)
        {
            ...
            if (((tbl[i] & 0x1C) >> 2) < 6)
            {
                ...
                p_addr = (void *)(((tbl[i] << 4) & ~mask) | ((air_uptr_t)v_addr & mask));
            }
