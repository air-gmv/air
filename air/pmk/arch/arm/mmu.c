...
static air_u32_t arm_get_mmu_permissions(air_u32_t permissions, air_u32_t level, air_u32_t type)
{
    ...
    air_u32_t access_mask =
        (0b000 << MMU_TEX_OFFSET(level, type)) | MMU_ACCESS_A(level, type) | MMU_ATTR_S(level, type);
    ...
    case PMK_MMU_R:
        access_mask |= MMU_ACCESS_RO(level, type) | MMU_ACCESS_UP(level, type) | MMU_ACCESS_XN(level, type) |
                       MMU_ACCESS_NG(level, type);
        break;

    case PMK_MMU_R | PMK_MMU_W:
        access_mask |= MMU_ACCESS_UP(level, type) | MMU_ACCESS_XN(level, type) | MMU_ACCESS_NG(level, type);
        break;

    case PMK_MMU_R | PMK_MMU_E:
        access_mask |= MMU_ACCESS_RO(level, type) | MMU_ACCESS_UP(level, type) | MMU_ACCESS_NG(level, type);
        break;

    case PMK_MMU_R | PMK_MMU_W | PMK_MMU_E:
        access_mask |= MMU_ACCESS_UP(level, type) | MMU_ACCESS_NG(level, type);
        break;
    ...
    case PMK_MMU_SR:
        access_mask |= MMU_ACCESS_RO(level, type) | MMU_ACCESS_XN(level, type);
        break;
...
static air_sz_t arm_mmu_map_l2(air_uptr_t *pt_ptr, void *p_addr, void *v_addr, air_u32_t unit, air_sz_t size,
                               air_u32_t permissions, air_u32_t n)
{
    ...
    printk("\n        Level 2\n"
           ...
           "          ptd[%03i]        = 0x%08x\n"
           "          ptd[%03i]        = 0x%08x\n\n",
           pt_ptr, initial_idx, pt_ptr[initial_idx], idx, pt_ptr[idx]);
#endif

    return consumed;
}

static void arm_mmu_map_l1(air_uptr_t *ttbr, void *p_addr, void *v_addr, air_sz_t unit, air_sz_t size,
                           air_u32_t permissions, air_u32_t n)
{
    ...
    while (size > 0 && idx < ln_entries[n] - 1)
    {
        ...
        if (unit < ln_unit[n])
        {
            ...
            default:
                ...
                ttbr[idx] = (((air_u32_t)pt_ptr & 0xfffffc00) | (DEFAULT_DOMAIN << 5) | MMU_PTD);
                break;
            }
        ...
        else
        {
            ...
            ttbr[idx] = (((air_u32_t)p_addr & 0xfff00000) | arm_permissions | (DEFAULT_DOMAIN << 5) | MMU_SECTION);
    ...
    printk("\n        Level 1\n"
           ...
           "          ttbr[%04i]      = 0x%08x\n"
           "          ttbr[%04i]      = 0x%08x\n\n",
           ttbr, initial_idx, ttbr[initial_idx], idx, ttbr[idx]);
#endif
}
...
void arm_segregation_init(void)
{
    ...
    arch_configuration_t *configuration = (arch_configuration_t *)pmk_get_arch_configuration();
...
void arm_map_memory(arm_mmu_context_t *ctrl, void *p_addr, void *v_addr, air_sz_t size, air_sz_t unit,
                    air_u32_t permissions)
{
    ...
    printk("       ttbr0              = 0x%08x\n"
           ...
           "       Size               = 0x%08x\n"
           "       Unit               = 0x%08x\n",
           ctrl->ttbr0, (air_u32_t)p_addr, (air_u32_t)v_addr, (air_u32_t)v_addr + (air_u32_t)size, size, unit);
#endif
...
air_u32_t arm_segregation_memcpy(arm_core_context_t *core_ctx, void *dst, void *src, air_sz_t size)
{
...
air_u32_t arm_copy_to_user(arm_core_context_t *core_ctx, void *dst, void *src, air_sz_t size)
{
...
air_u32_t arm_copy_from_user(arm_core_context_t *core_ctx, void *dst, void *src, air_sz_t size)
{
