 ...
 * @ingroup bsp_leon_amba
 */
static void amba_search_ahb_slaves(amba_confarea_t *amba_conf, air_uptr_t io_area)
{
 ...
 * @ingroup bsp_leon_amba
 */
static void amba_search_apb_slaves(amba_confarea_t *amba_conf, air_uptr_t io_area)
{
 ...
 * @ingroup
 */
static void amba_search_bridge(amba_confarea_t *amba_conf, air_uptr_t io_area)
{
 ...
 * @ingroup bsp_leon_amba
 */
static void amba_search_ahb_masters(amba_confarea_t *amba_conf, air_uptr_t io_area)
{
    ...
    for (i = scanned_devices; i < amba_conf->ahb_slaves.count; ++i)
    {
        ...
        if (get_vendor(conf) == VENDOR_GAISLER && get_device(conf) == GAISLER_AHB2AHB)
        {
        ...
        if (get_vendor(conf) == VENDOR_GAISLER && get_device(conf) == GAISLER_APBMST)
        {
 ...
 */
static inline void get_ahb_device(amba_confarea_t *amba_conf, amba_ahb_table_t *table, int index, amba_ahb_dev_t *dev)
{
    ...
    for (i = 0; i < amba_conf->apb_slaves.count; ++i)
    {
        ...
        if (get_vendor(conf) == vendor && get_device(conf) == device)
        {
            ...
            if (index == count || index == -1)
            {
                ...
                addr = amba_conf->apb_slaves.apb_masters[i] | (((addr & 0xFFF00000) >> 12) & ((addr & 0xFFF0) << 4));
                dev->start = addr;
