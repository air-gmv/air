...
MISRA_RULES = os.path.dirname(os.path.abspath(__file__)) + '/misra-rules-all.txt'
...
def runCmd(cmd, cwd=".") -> bool:
    try:
        ...
        print(f'Status : FAIL, returncode: {exc.returncode}, output: {exc.output}')
    else:
        if output != "":
            print(f'Out: {output}')
...
class CodingRulesRunner(object):
    ...
    def run(self):
        ...
        subprocess.check_call(['rm', '-f', OUT_INCLUDES], cwd=self.output_folder)
        subprocess.check_call(['rm', '-f', OUT_CPPCHECK], cwd=self.output_folder)

        with open('%s/%s' % (self.output_folder, OUT_INCLUDES), 'a') as f:
            subprocess.check_call('find "%s/" -name "*.[h]" | xargs -I{} dirname {} | uniq' % self.project_folder, shell=True, stdout=f)

        if self.src_file_list:
            with open('%s/%s' % (self.output_folder, OUT_CPPCHECK), 'a') as f:
                subprocess.check_call(CPPCHECK_EXE + self.CPPCHECK_FLAGS + self.PREPROCESSOR_FLAGS + self.excluded + ['--xml', f'--addon={MISRA_CONFIG}',f'--includes-file={self.output_folder}/{OUT_INCLUDES}'] + self.src_file_list, stderr=f)
        else:
            with open('%s/%s' % (self.output_folder, OUT_CPPCHECK), 'a') as f:
                subprocess.check_call(CPPCHECK_EXE + self.CPPCHECK_FLAGS + self.PREPROCESSOR_FLAGS + self.excluded + ['--xml', f'--addon={MISRA_CONFIG}',f'--includes-file={self.output_folder}/{OUT_INCLUDES}', self.project_folder], stderr=f)
        
        runCmd(f'python3 cppcheck-htmlreport.py --file {self.output_folder}/{OUT_CPPCHECK} --report-dir {self.output_folder}/html --source-dir {self.project_folder}', cwd=os.path.dirname(os.path.abspath(__file__)))
...
def run(project_folder, output_folder, config_file, exclude_file, src_file_list):
    CodingRulesRunner(project_folder, output_folder, config_file, exclude_file, src_file_list).run()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.configuration):
        print(f"Error, configuration file {args.configuration} does not exists")
        parser.print_help()
        sys.exit(-1)

    os.makedirs(OUTPUT_FOLDER, exist_ok=True)

    run(SOURCE_FOLDER, OUTPUT_FOLDER, args.configuration, args.exclude, args.src_file_list)
